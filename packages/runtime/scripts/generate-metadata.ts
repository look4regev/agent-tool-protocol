...
async function main() {
	...
	// Step 1: Extract API metadata (ts-morph for decorators only)
	...
	// Step 2: Filter to only custom types that exist in our source files
	const customTypes = Array.from(referencedTypes).filter((typeName) => {
		// Skip generic params like T, U, V
		if (typeName.length === 1 && typeName === typeName.toUpperCase()) {
			return false;
		}
		// Only include types that exist in our source files
	...
	// Step 3: Extract each custom type
